    ...
    allBets.map(async (el) => {
      ...
      if (el.result === matchDB.result_match) {
        ...
        await Bet.update({
          ...
          final_profit: Math.round(el.money_bet * actualCoef),
        },
          {
            where: {
              id: el.id,
...
router.get('/mailResultsStage', async (req, res, next) => {

  try {

    let simulate = req.query.sim
    let stageMatch = req.query.resetMatchId

     if (simulate === "reset") {
      await Bet.update({
        condition: stageMatch,
      },
        {
          where: {
            condition: `finishedStage_${stageMatch}`,
          }
        });
      ...
      setTimeout(async function () {
        ...
        for (let i = 0; i < allBets.length; i++) {
          ...
          if (allBets[i].final_profit !== null && allBets[i].final_profit > -1) {
            ...
            if (allBets[i].final_profit === 0 && Number(allBets[i].condition) > 0 ) {

              var foundMatch = await Stage_fixture.findByPk(Number(allBets[i].condition))
              ...
              await Bet.update({
                condition: `finishedStage_${Number(allBets[i].condition)}`,
              },
                {
                  where: {
                    id: allBets[i].id,
